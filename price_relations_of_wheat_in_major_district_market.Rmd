...
## Precipitation of Nepalese district markets
...
## International and India's bordering region prices
...
## Descriptive time series summary
...
```{r lm1-without-time-producer}
# fit a lm as it is
price_lm11 <- lm(data = ppf_ts_wide, formula = lp_npl_krp ~ 
                 # date + 
                 lp_canada +
                 lp_india +
                 lp_npl_ck +
...
# fit a lm as it is
price_lm12 <- lm(data = ppf_ts_wide, formula = lp_npl_ck ~ 
                  lp_canada +
                  lp_india +
                  lp_npl_krp +
                  precip_npl_ck +
...
car::residualPlot(price_lm11, main = "Producer districts")
car::residualPlot(price_lm12, main = "Consumer districts")
```
...
price_lm21 <- lm(data = ppf_ts_wide, formula = lp_npl_krp ~ 
                 date +
                 lp_canada +
                 lp_india +
                 lp_npl_ck +
...
price_lm22 <- lm(data = ppf_ts_wide, formula = lp_npl_ck ~ 
                 date +
                 lp_canada +
                 lp_india +
                 lp_npl_krp +
...
car::residualPlot(price_lm21, main = "Producer districts")
car::residualPlot(price_lm22, main = "Consumer districts")
```
...
```{r lm2-bptest}
map_dfr(list(price_lm21, price_lm22), 
        ~lmtest::bptest(.x) %>% 
          tidy()) %>% 
...
diff(ppf_ts_box[, c(8, 9, 10, 11, 5, 6, 12)], lag = ndiffs(ppf_ts_box[, c(8, 9, 10, 11, 5, 6, 12)], 
...
price_lm31 <- lm(data = ppf_ts_wide,
                formula = lp_npl_krp ~
                 date +
                 lp_canada +
                 lp_india +
...
```{r lm3-consumer-market}
price_lm32 <- lm(data = ppf_ts_wide,
                formula = lp_npl_ck ~
                 date +
                 lp_canada +
                 lp_india +
...
```{r lm3-anova-consumer-market-tab}
price_lm32 %>% 
  anova() %>% 
  broom::tidy() %>% 
...
ca.jo(ppf_ts_box[,c("lp_canada", "lp_india", "lp_npl_ck", "lp_npl_krp", "lp_fuel")],
              K=2, spec="transitory",
              type = "trace",
              ecdet = "trend" # "trend" type determinsitic regressor only
              ) %>% 
  summary() %>% 
  slot("cval") %>% 
...
```{r wheat-cajo-test-without-ex, results='asis'}
# use data subsetted for values without NA
# # fit using urca::ca.jo
# map2(.x = c("const", "trend"), 
#      .y = c("longrun", "transitory"),
#     ~urca::ca.jo(ppf_ts_box[,c("lp_canada", "lp_india", "lp_npl_ck", "lp_npl_krp", "lp_fuel")], type = "trace", 
#             ecdet = .x, K = 2, spec = .y, 
#             season = 12, 
#             dumvar = ppf_ts_box[,c("precip_npl_krp_trend", "precip_npl_ck_trend")])) %>% 
#   .[[1]] %>% 
#   summary()

# both longrun and transitory give similar result 

# r = number of cointegrating vector. It is best kept at 2 for ease of intrepretation. greater the number of 
# cointegrating vectors there are, difference between "transitory" and "longrun" relationships are more pronounced

ppf_ts_vecm_noex_both <- VECM(ppf_ts_box[,c("lp_canada", "lp_india", "lp_npl_ck", "lp_npl_krp", "lp_fuel")], 
     lag=1, estim="ML", r=2, 
     ...
     include = "const",
     LRinclude = "both" # "both" trend and "constant" type deterministic regressors to included in the long-term relationship. 
     )

# coeffecients
ppf_ts_vecm_noex_both$model.specific$beta %>% 
  as_tibble(rownames = "terms") %>% 
  knitr::kable(booktab = TRUE, caption = 'ML estimation of VECM model coefficients with two cointegrating vectors, with lag order 1 and "both" (trend and constant) type deterministic regressors included in the long-term relationship.')

```

\blandscape

```{r wheat-cajo-test-without-ex-vecm, results='asis'}

# # including "both" types of deterministic regressors in VECM part may be better than "trend" only.
# # because using "both" gives stronger beta coefficients in first cointegrating eigenvector, principally
# # verify that with inference. 
# cajorls(ca.jo(ppf_ts_box[,c("lp_canada", "lp_india", "lp_npl_ck", "lp_npl_krp", "lp_fuel")], 
#               K=2, spec="transitory", 
#               type = "trace", 
#               ecdet = "trend" # "trend" type determinsitic regressor only  
#               ), r=2)$beta %>% 
...
##export to Latex
ppf_ts_vecm_noex_none_latex <- VECM(ppf_ts_box[,c("lp_canada", "lp_india", "lp_npl_ck", "lp_npl_krp", "lp_fuel")], 
     lag=1, estim="ML", r=2, 
     # Type of deterministic regressors. "trend" and "both" types cannot be fitted (result in singularity) 
     # by default `ca.jo()` procedure fits "constant" type deterministic regressors
     # setting include = "none" gives different values
     include = "const",
     LRinclude = "none" # "both" trend and "constant" type deterministic regressors to included in the long-term relationship. 
     )
cat(toLatex(ppf_ts_vecm_noex_none_latex, label = "eqn:vecm-noex-none", digits = 2)[-c(1:4)], sep = "\n") # plus a trend term both trend and constant term in obtain results
# toLatex(summary(ppf_ts_vecm_noex_none_latex))
options("show.signif.stars"=FALSE)
cat(toLatex(summary(ppf_ts_vecm_noex_none_latex), parenthese="Pvalue", digits = 2)[-c(1:4)], sep = "\n")
...
```{r wheat-cajo-test-withex-vecm, results='asis'}
# lp_canada.l1 and lp_india.l1 have different beta values with the two tests (VECM and ca.jo), more so if r >= 3.
# "longrun" estimates of `ca.jo()` are near `VECM()` estimates than the "transitory" estimates, when r is large
# "transitory" estimates of `ca.jo()` are near `VECM()` estimates than the "longrun" estimates, when r is small.

# # including exogenous regressors with trend type deterministic term.
# for the sake of easy comparison we use "trend" type
# exogeneous regressors -- precip_npl_ck_trend and precip_npl_krp_trend
ppf_ts_vecm_ex_trend <- VECM(ppf_ts_box[,c("lp_canada", "lp_india", "lp_npl_ck", "lp_npl_krp", "lp_fuel")], 
     lag=1, estim="ML", r=2, 
     # Type of deterministic regressors. "trend" and "both" types cannot be fitted (result in singularity) 
     # by default `ca.jo()` procedure fits "constant" type deterministic regressors
     # setting include = "none" gives different values
     include = "const",
     exogen = ppf_ts_box[,c("precip_npl_krp_trend", "precip_npl_ck_trend")],
     LRinclude = "trend" # type of deterministic regressors to include in the long-term relationship. 
)

ppf_ts_vecm_ex_trend$model.specific$beta %>% 
  as_tibble(rownames = "terms") %>% 
  knitr::kable(booktab = TRUE, caption = 'ML estimation of VECM model coefficients with two cointegrating vectors, with lag order 1, with exogeneous regressors and "trend" type deterministic regressors included in the long-term relationship.')

##export to Latex
ppf_ts_vecm_ex_none_latex <- VECM(ppf_ts_box[,c("lp_canada", "lp_india", "lp_npl_ck", "lp_npl_krp", "lp_fuel")], 
     lag=1, estim="ML", r=2, 
     # Type of deterministic regressors. "trend" and "both" types cannot be fitted (result in singularity) 
     # by default `ca.jo()` procedure fits "constant" type deterministic regressors
     # setting include = "none" gives different values
...
cat(toLatex(ppf_ts_vecm_ex_none_latex, label = "eqn:vecm-ex-none", digits = 2)[-c(1:4)], sep = "\n")
# toLatex(summary(ppf_ts_vecm_ex_none_latex))
options("show.signif.stars"=FALSE)
cat(toLatex(summary(ppf_ts_vecm_ex_none_latex), parenthese="Pvalue", digits = 2)[-c(1:4)], sep = "\n")

# # including exogenous regressors with both type determinstic term
ppf_ts_vecm_ex_both <- VECM(ppf_ts_box[,c("lp_canada", "lp_india", "lp_npl_ck", "lp_npl_krp", "lp_fuel")], 
                             lag=1, estim="ML", r=2, 
                             # Type of deterministic regressors. "trend" and "both" types cannot be fitted (result in singularity) 
                             # by default `ca.jo()` procedure fits "constant" type deterministic regressors
                             # setting include = "none" gives different values
                             include = "const",
                             exogen = ppf_ts_box[,c("precip_npl_krp_trend", "precip_npl_ck_trend")],
                             LRinclude = "both" # type of deterministic regressors to include in the long-term relationship. 
)
...
```{r wheat-cajo-test-withex-summary, results='asis'}
ppf_ts_vecm_ex_both$model.specific$beta %>% 
  as_tibble(rownames = "terms") %>%
  knitr::kable(booktab = TRUE, caption = 'ML estimation of VECM model coefficients with two cointegrating vectors, with lag order 1, with exogeneous regressors and with "both" (trend and constant) type deterministic regressors included in the long-term relationship.')
